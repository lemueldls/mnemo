...
use tiny_skia as sk;
use typst_library::{
    layout::{Axes, Point, Ratio, Size},
    visualize::{Color, Gradient, Paint, RelativeTo, Tiling},
};
...
impl<'a> GradientSampler<'a> {
    pub fn new(gradient: &'a Gradient, state: &State, item_size: Size, on_text: bool) -> Self {
...
impl<'a> TilingSampler<'a> {
    pub fn new(tilings: &'a Tiling, pixmap: &'a sk::Pixmap, state: &State, on_text: bool) -> Self {
...
pub fn to_sk_paint<'a>(
    ...
    match paint {
        ...
        Paint::Gradient(gradient) => {
            ...
            let width = (container_size.x.to_f32().abs() * state.pixel_per_pt).ceil() as u32;
            let height = (container_size.y.to_f32().abs() * state.pixel_per_pt).ceil() as u32;
...
pub fn to_sk_color(color: Color) -> sk::Color {
    ...
    sk::Color::from_rgba(r, g, b, a).expect("components must always be in the range [0..=1]")
}
