/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const __wbg_fileid_free: (a: number, b: number) => void;
export const __wbg_themecolors_free: (a: number, b: number) => void;
export const __wbg_typststate_free: (a: number, b: number) => void;
export const rgb_new: (a: number, b: number, c: number) => number;
export const rgb_toString: (a: number, b: number) => void;
export const start: () => void;
export const themecolors_new: (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number, j: number, k: number, l: number, m: number, n: number, o: number, p: number, q: number, r: number, s: number, t: number) => number;
export const typststate_autocomplete: (a: number, b: number, c: number, d: number) => number;
export const typststate_checkHTML: (a: number, b: number, c: number, d: number, e: number, f: number) => number;
export const typststate_checkPaged: (a: number, b: number, c: number, d: number, e: number, f: number) => number;
export const typststate_compileHTML: (a: number, b: number, c: number, d: number, e: number, f: number) => number;
export const typststate_compilePaged: (a: number, b: number, c: number, d: number, e: number, f: number) => number;
export const typststate_createFileId: (a: number, b: number, c: number) => number;
export const typststate_createSourceId: (a: number, b: number, c: number, d: number, e: number) => number;
export const typststate_highlight: (a: number, b: number, c: number, d: number, e: number) => void;
export const typststate_hover: (a: number, b: number, c: number, d: number, e: number) => void;
export const typststate_insertFile: (a: number, b: number, c: number, d: number) => void;
export const typststate_insertSource: (a: number, b: number, c: number, d: number) => void;
export const typststate_installFont: (a: number, b: number, c: number) => void;
export const typststate_installPackage: (a: number, b: number, c: number, d: number, e: number, f: number) => void;
export const typststate_jumpHTML: (a: number, b: number, c: number, d: number) => number;
export const typststate_jumpPaged: (a: number, b: number, c: number, d: number) => number;
export const typststate_new: () => number;
export const typststate_removeFile: (a: number, b: number) => void;
export const typststate_renderHtml: (a: number, b: number, c: number, d: number, e: number, f: number) => number;
export const typststate_resize: (a: number, b: number, c: number, d: number, e: number, f: number) => number;
export const typststate_setCodeFont: (a: number, b: number, c: number, d: number) => void;
export const typststate_setFont: (a: number, b: number, c: number, d: number) => void;
export const typststate_setLocale: (a: number, b: number, c: number, d: number) => void;
export const typststate_setMathFont: (a: number, b: number, c: number, d: number) => void;
export const typststate_setTheme: (a: number, b: number, c: number) => void;
export const __wbg_rgb_free: (a: number, b: number) => void;
export const lut_inverse_interp16: (a: number, b: number, c: number) => number;
export const qcms_profile_precache_output_transform: (a: number) => void;
export const lut_interp_linear16: (a: number, b: number, c: number) => number;
export const qcms_enable_iccv4: () => void;
export const qcms_profile_is_bogus: (a: number) => number;
export const qcms_transform_data_bgra_out_lut: (a: number, b: number, c: number, d: number) => void;
export const qcms_transform_data_bgra_out_lut_precache: (a: number, b: number, c: number, d: number) => void;
export const qcms_transform_data_rgb_out_lut: (a: number, b: number, c: number, d: number) => void;
export const qcms_transform_data_rgb_out_lut_precache: (a: number, b: number, c: number, d: number) => void;
export const qcms_transform_data_rgba_out_lut: (a: number, b: number, c: number, d: number) => void;
export const qcms_transform_data_rgba_out_lut_precache: (a: number, b: number, c: number, d: number) => void;
export const qcms_transform_release: (a: number) => void;
export const qcms_white_point_sRGB: (a: number) => void;
export const __wbindgen_export: (a: number, b: number) => number;
export const __wbindgen_export2: (a: number, b: number, c: number, d: number) => number;
export const __wbindgen_export3: (a: number, b: number, c: number) => void;
export const __wbindgen_add_to_stack_pointer: (a: number) => number;
export const __wbindgen_start: () => void;
