...
use typst::{
    foundations::Smart,
    layout::{Abs, Axes},
    visualize::{ExchangeFormat, Image, ImageKind, ImageScaling, PdfImage, RasterFormat},
};

use super::{SVGRenderer, State, SvgMatrix};

impl SVGRenderer<'_> {
    /// Render an image element.
    pub(super) fn render_image(&mut self, state: &State, image: &Image, size: &Axes<Abs>) {
...
fn pdf_to_svg(pdf: &PdfImage) -> String {
    ...
    let interpreter_settings = InterpreterSettings {
        font_resolver: Arc::new(move |query| {
            match query {
