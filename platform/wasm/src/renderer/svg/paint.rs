...
use typst::{
    foundations::Repr,
    layout::{Angle, Axes, Frame, Quadrant, Ratio, Size, Transform},
    utils::hash128,
    visualize::{Color, FillRule, Gradient, Paint, RatioOrAngle, Tiling},
};
use xmlwriter::XmlWriter;

use super::{Id, SVGRenderer, State, SvgMatrix, SvgPathBuilder};
...
impl SVGRenderer<'_> {
    ...
    pub(super) fn push_gradient(&mut self, gradient: &Gradient, size: Size, ts: Transform) -> Id {
        ...
        self.gradient_refs
            .insert_with(hash128(&(gradient_id, ts)), || {
                GradientRef {
                    id: gradient_id,
                    kind: gradient.into(),
                    transform: ts,
    ...
    pub(super) fn push_tiling(&mut self, tiling: &Tiling, size: Size, ts: Transform) -> Id {
        ...
        self.tiling_refs.insert_with(hash128(&(tiling_id, ts)), || {
            TilingRef {
                id: tiling_id,
                transform: ts,
                ratio: Axes::new(
