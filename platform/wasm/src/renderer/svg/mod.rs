...
use paint::{GradientRef, SVGSubGradient, TilingRef};
...
use typst::{
    ...
    model::Destination,
    utils::hash128,
    visualize::{Geometry, Gradient, Tiling},
};
use xmlwriter::XmlWriter;

use crate::renderer::paged::FrameBlock;
...
pub fn svg_frame(items: Vec<FrameBlock>) -> String {
    let mut renderer = SVGRenderer::new();
...
impl<'a> SVGRenderer<'a> {
    ...
    fn with_options(options: xmlwriter::Options, introspector: Option<&'a Introspector>) -> Self {
        SVGRenderer {
    ...
    fn render_frame(&mut self, state: &State, frame: &Frame) {
        ...
        for (pos, item) in frame.items() {
            ...
            match item {
                ...
                FrameItem::Image(image, size, _) => self.render_image(&state, image, size),
    ...
    fn render_group(&mut self, state: &State, group: &GroupItem) {
        ...
        if let Some(label) = group.label {
            self.xml
                .write_attribute("data-typst-label", &label.resolve());
        }
    ...
    fn render_link(&mut self, state: &State, dest: &Destination, size: Size) {
        ...
        if !state.transform.is_identity() {
            self.xml
                .write_attribute("transform", &SvgMatrix(state.transform));
        }
