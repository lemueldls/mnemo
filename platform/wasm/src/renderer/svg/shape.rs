...
use super::{SVGRenderer, State, SvgMatrix, SvgPathBuilder, paint::ColorEncode};
...
impl SVGRenderer<'_> {
    ...
    pub(super) fn write_stroke(
        ...
        self.xml
            .write_attribute("stroke-linecap", match stroke.cap {
                LineCap::Butt => "butt",
                LineCap::Round => "round",
                LineCap::Square => "square",
            });
        self.xml
            .write_attribute("stroke-linejoin", match stroke.join {
...
fn convert_geometry_to_path(geometry: &Geometry) -> EcoString {
    let mut builder = SvgPathBuilder::with_translate(Point::new(Abs::zero(), Abs::zero()));
