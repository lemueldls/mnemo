...
use typst::{
    ...
    text::{Font, TextItem, color::colr_glyph_to_svg},
    utils::hash128,
    visualize::{ExchangeFormat, FillRule, Image, Paint, RasterImage, RelativeTo},
};

use super::{SVGRenderer, State, SvgMatrix, SvgPathBuilder};
...
impl SVGRenderer<'_> {
    ...
    fn render_bitmap_glyph(
        ...
        let id = self.glyphs.insert_with(glyph_hash, || {
            ...
            let url = super::image::convert_image_to_base64_url(&image);
            let ts =
    ...
    fn render_outline_glyph(
        ...
        self.xml
            .write_attribute_fmt("y", format_args!("{y_offset}"));
        self.write_fill(
            &text.fill,
            FillRule::default(),
...
fn convert_svg_glyph_to_base64_url(font: &Font, id: GlyphId) -> Option<EcoString> {
    ...
    if last_viewbox.is_none() {
        ...
        svg_str.insert_str(
            ...
            format!(
                r#" viewBox="0 {} {width} {height}""#,
                -origin_ascender.to_pt()
            )
            .as_str(),
        );
    }
