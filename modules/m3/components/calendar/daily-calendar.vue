...
onMounted(() => {
  ...
  useIntervalFn(
    () => {
      ...
      caret.style.top = `${((now.getHours() + now.getMinutes() / 60) / 24) * scrollHeight.value
        }px`;
    },
    1000 * 60,
  ...
  <div class="h-full flex flex-1 flex-col gap-4 overflow-hidden">
    ...
    <div ref="container" class="flex flex-1 gap-4 overflow-auto">
      ...
      <div class="relative flex-1">
        <m3-theme v-for="({ spaceId, from, to }, i) in todaysSchedule" :key="i" :color="spaces![spaceId]!.color"
