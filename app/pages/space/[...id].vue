...
<template>
  <m3-theme id="space-page" :color="space.color" :dark="dark">
    <sticky-note
      ...
      :note="note"
      @mousedown="() => {
        // activeStickyNotes[i] = activeStickyNotes.pop();
        // activeStickyNotes.push(note);
        
        // const temp = activeStickyNotes.at(-1);
        // activeStickyNotes[activeStickyNotes.length - 1] = note;
        // activeStickyNotes[i] = temp;

        const lastNote = activeStickyNotes.at(-1);
        const currentNote = activeStickyNotes[i];
        if (lastNote) {
          // activeStickyNotes[i] = lastNote;
          // if (currentNote) activeStickyNotes[activeStickyNotes.length - 1] = currentNote;
        }
      }"
      @close="
        activeStickyNotes = activeStickyNotes.filter((n) => n.id !== note.id)
        // activeStickyNotes.splice(i, i + 1)
        // () => delete activeStickyNotes[i]
      "
    />
    ...
    <m3-page>
      ...
      <md-dialog :open="preludeOpen" @closed="preludeOpen = false">
        <editor :space-id="spaceId" v-model="currentNote.id" kind="prelude" class="h-full flex-1" />
      </md-dialog>
