...
export const useReview = createSharedComposable(async (amount: MaybeRefOrGetter<number>) => {
  ...
  watchDebounced(
    [amount, spaceNotes],
    async (results) => {
      const [max, spaceNotes] = results as [number, { spaceId: string; notes: DailyNote[] }[]];
      ...
      await Promise.all(
        spaceNotes.map(async ({ spaceId, notes }) => {
          ...
          for (let i = end; i >= 0 && notesToReview.length < max; i--) {
            ...
            const content = await getStorageItem(`spaces/${spaceId}/daily/${noteId}.typ`);
